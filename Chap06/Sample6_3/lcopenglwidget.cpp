...
//    glViewport(0,0,width()/2,height());
    ...
    projection.perspective(m_camera.Zoom,(float)width()/height(),0.1f,100);//透视投影矩阵
...
{
    float deltaTime = TIMEOUTMSEC/1000;
    ...
    case Qt::Key_W:
        ...
        break;
    case Qt::Key_S:
        m_camera.ProcessKeyboard(RIGHT,deltaTime);break;
        break;
    case Qt::Key_D:
        m_camera.ProcessKeyboard(RIGHT,deltaTime);break;
        break;
    case Qt::Key_A:
        m_camera.ProcessKeyboard(LEFT,deltaTime);break;
        break;
...
void LCOpenGLWidget::mouseMoveEvent(QMouseEvent *_event)
{
    ...
    m_deltaPos=currentPos-lastPos;
    lastPos=currentPos;

    m_camera.ProcessMouseMovement(m_deltaPos.x(),-m_deltaPos.y());
...
{
    m_camera.ProcessMouseScroll(_event->angleDelta().y()/120);
    update();
}

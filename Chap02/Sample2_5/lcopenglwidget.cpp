...
{
    m_timer.start(100); // 刷新频率为100赫兹，即一秒钟刷新10次
...
void LCOpenGLWidget::slotOnTimeout()
{
    if(m_shape==None){
        return;
    }
    ...
    // uniform从CPU(即我们写的代码)向GPU中的着色器发送数据
    //  uniform是全局的(Global)，可以被任意着色器程序在任意阶段访问
    // 设置名为ourColor的uniform的参数
    m_shaderProgram.setUniformValue("ourColor",0.0f, greenValue, 0.0f, 1.0f);

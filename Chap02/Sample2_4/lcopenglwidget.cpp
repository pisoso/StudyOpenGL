...
//    GLint posLocation = m_shaderProgram.attributeLocation("aPos"); //先询问aPos的位置
    GLint posLocation = 2;
    m_shaderProgram.bindAttributeLocation("aPos",posLocation); //将aPos的位置改为posLocation，就不用在外面用layout定义了
    // 告知显卡如何解析缓冲里的属性值
    // 第一个参数指定要配置的顶点属性，最多16个，可以是位置，颜色，纹理等等
    glVertexAttribPointer(posLocation,3,GL_FLOAT,GL_FALSE,3*sizeof(float),(void*)0);
    // 开启VAO管理的第(posLocation+1)个属性值
